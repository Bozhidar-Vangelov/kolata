...
export default function VignettePage() {
  ...
  const [records, setRecords] = useState<Vignette[]>([]);
  ...
  function openEdit(record: Vignette) {
    setEditing(record);
    setShowForm(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (editing) {
      await supabase.from("vignette").update(payload).eq("id", editing.id);
    } else {
      await supabase.from("vignette").insert(payload);
    }
  ...
  async function handleDelete(id: string) {
    ...
    const supabase = createClient();
    await supabase.from("vignette").delete().eq("id", id);
    load();
  }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">{t("vignette.title")}</h1>
